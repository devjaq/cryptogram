...
let updateMsg = () => {
  ...
  for (let i = 0; i < workingCopy.length; i++) {
    if (
      ...
      newResult[i] = `<span class="unknown ${workingCopy[i]}">${
        workingCopy[i]
      }</span>`;
    }
...
for (let i = 0; i < 26; i++) {
  ...
  letterInput.innerHTML = `${
    ...
    alpha[i]
  }" onfocus="highlight(id, value)" onblur="letterChange(id, value)">`;
...
let letterChange = (oldLetter, newLetter) => {
  ...
  for (let i = 0; i < codedMsg.length; i++) {
    if (
      codedMsg[i] === oldLetter &&
      newLetter !== null &&
      newLetter !== "" &&
      newLetter !== " "
    ) {
      workingCopy.splice(i, 1, `<span class="${oldLetter}" unknown=false>${newLetter}</span>`);
      letter.setAttribute("newletter", newLetter);
    } else if (
      (codedMsg[i] === oldLetter && newLetter === null) ||
      (codedMsg[i] === oldLetter && newLetter === "") ||
      (codedMsg[i] === oldLetter && newLetter === " ")
    ) {
      workingCopy.splice(i, 1, `<span class="${oldLetter} unknown" unknown=true>${oldLetter}</span>`);
